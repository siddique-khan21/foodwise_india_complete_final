...
async function loadCompostDonations() {
    const result = await getCompostDonations();
    const container = document.getElementById('donationsList');
    
    console.log('🌿 Compost API Result:', result); // ADD THIS
    
    if (result.error || !result.donations || result.donations.length === 0) {
        container.innerHTML = '<p>No composting donations available.</p>';
        return;
    }
    ...
    container.innerHTML = result.donations.map(donation => `
        <div class="donation-card">
            ...
            <button class="btn btn-accept" onclick="acceptCompostDonation(${donation.id}, '${donation.donor_phone}')">Accept for Composting</button>
        </div>
    `).join('');
}

async function acceptCompostDonation(donationId, donorPhone) {
    const user = JSON.parse(localStorage.getItem('currentUser'));
    const result = await acceptDonationAPI(donationId, user.id);
    
    if (result.success) {
        showMessage(`✅ Composting material accepted!\n\nContact Donor: ${donorPhone}\n\nCoordinate pickup for composting.`);
        loadCompostDonations();
    } else {
